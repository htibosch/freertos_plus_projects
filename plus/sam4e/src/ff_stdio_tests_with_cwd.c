...
int iReturned, iExpectedReturn;
	...
	if( strcmp (pcMountPath, "/test/1") == 0 )
	{
...
//		FF_GetFreeSize( pxRAMDisk->pxIOManager, &xError );
		...
		ullFreeSectors = pxSDDisk->pxIOManager->xPartition.ulFreeClusterCount * pxSDDisk->pxIOManager->xPartition.ulSectorsPerCluster;
		iPercentageFree = ( int ) ( ( ramHUNDRED_64_BIT * ullFreeSectors + pxSDDisk->pxIOManager->xPartition.ulDataSectors / 2 ) /
			( ( uint64_t )pxSDDisk->pxIOManager->xPartition.ulDataSectors ) );
		...
		FF_PRINTF( "FreeSize     %lu / %lu (%lu .. %lu) KB ( %d perc free )\n",
			ulFreeSizeKB,
			pxSDDisk->pxIOManager->xPartition.ulDataSectors,
