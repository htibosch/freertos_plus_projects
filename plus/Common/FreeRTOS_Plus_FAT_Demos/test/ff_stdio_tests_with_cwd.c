...
char cReadBuffer[ 45 ];
	...
	/* If using a RAM disk the files will not already exist.  This assumes the
	word "ram" is used in the mount name. */
...
char *pcRAMBuffer, *pcFileName;
	...
	/* If using a RAM disk the files will not already exist.  This assumes the
	word "ram" is used in the mount name. */
	if( ( strstr( pcMountPath, "RAM" ) != NULL ) || ( strstr( pcMountPath, "ram" ) != NULL ) )
	{
