...
/*-----------------------------------------------------------*/
	...
	char pcReadString[ 24 ], pcExpectedString[ 24 ], *pcReturned;
	const char *pcMaximumStringLength = "Test string 999\n";
		...
		for( iString = 0; iString < iMaxStrings; iString++ )
		{
			...
			can be checked. */
			snprintf( pcExpectedString, sizeof pcExpectedString, "%s %d\n", pcStringStart, iString );
...
int iReturned, iExpectedReturn;
	...
	overwritten. */
	pcBuffer = ( char * ) pvPortMalloc( xBufferSize + xOverwriteCheckBytes + 512 ); // 512 * 3 + 37 + 512 = 2085
	configASSERT( pcBuffer );
	pulVerifyBuffer = ( uint32_t * ) pvPortMalloc( xBufferSize + 512 ); // 512 * 3 + 37 = 1573
	...
	if( strcmp (pcMountPath, "/ram/1") == 0 )
	{
		uint64_t ullFreeSectors;
		uint32_t ulFreeSizeKB;
		static uint32_t ulMinFreeSizeKB = 0ul;
		static uint32_t ulMaxFreeSizeKB = 0ul;
		FF_Error_t xError;
		int iPercentageFree;
...
#define ramHUNDRED_64_BIT			100ULL
		...
		FF_GetFreeSize( pxRAMDisk->pxIOManager, &xError );

		ullFreeSectors = pxRAMDisk->pxIOManager->xPartition.ulFreeClusterCount * pxRAMDisk->pxIOManager->xPartition.ulSectorsPerCluster;
		iPercentageFree = ( int ) ( ( ramHUNDRED_64_BIT * ullFreeSectors + pxRAMDisk->pxIOManager->xPartition.ulDataSectors / 2 ) /
			( ( uint64_t )pxRAMDisk->pxIOManager->xPartition.ulDataSectors ) );
		ulFreeSizeKB = ( uint32_t ) ullFreeSectors;
		if( ( ulMinFreeSizeKB == 0ul ) || ( ulMinFreeSizeKB > ulFreeSizeKB ) )
		{
			ulMinFreeSizeKB = ulFreeSizeKB;
		}
		if( ulMaxFreeSizeKB < ulFreeSizeKB )
		{
			ulMaxFreeSizeKB = ulFreeSizeKB;
		}

		FF_PRINTF( "FreeSize     %lu / %lu (%lu .. %lu) KB ( %d perc free )\n",
			ulFreeSizeKB,
			pxRAMDisk->pxIOManager->xPartition.ulDataSectors,
			ulMinFreeSizeKB, ulMaxFreeSizeKB, iPercentageFree );
	}
	...
	for( xSkippedBytes = 0; xSkippedBytes < xSizeIncrement; xSkippedBytes++ )
	{
		...
		memset( pulVerifyBuffer, 0x00, xBufferSize + 512 );
...
//	if( pxFile != NULL )
//	{
//		FF_PRINTF("ASSERT: ff_fopen (\"%s\", \"r\"): File should not open\n", pcFileName );
//		ff_fclose( pxFile );
//	}
