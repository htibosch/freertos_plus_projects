...
/* Stores the stack size passed into vStartSimple2TCPServerTasks() so it can be
reused when the server listening task creates tasks to handle connections. */
static uint16_t usUsedStackSize = 0;

/*-----------------------------------------------------------*/

void vStartSimple2TCPServerTasks( uint16_t usStackSize, UBaseType_t uxPriority )
{
...
const BaseType_t xBacklog = 20;
    ...
    xBindAddress.sin_port = 1000;//tcpechoPORT_NUMBER;
...
int32_t lBufSize;
	...
	the same buffer. */
  lBufSize = ipconfigTCP_MSS;
  pucRxBuffer = ( uint8_t * ) pvPortMalloc( lBufSize );
	...
	if( pucRxBuffer != NULL )
	{
      ...
      lBytes = lBufSize;
			...
			if( lBytes >= 0 )
			{
				...
				{
					lSent = FreeRTOS_send( xConnectedSocket, pucRxBuffer+lTotalSent, lBytes - lTotalSent, 0 );
					lTotalSent += lSent;
					//sentBytes+= lSent;
				}
  ...
  //xil_printf("prvServerConnectionInstance deleted.\r\n");
	vTaskDelete( NULL );
}
/*-----------------------------------------------------------*/
