                          ...
                          FF_Disk_t * pxDisk )
{
    ...
    configASSERT( ( pxDisk != NULL ) && /*_RB_ Could this be changed to an assert? */
...
eventLogAdd( "Read %u sectors", ( unsigned ) ulSectorCount );
...
eventLogAdd( "Write %u sectors", ( unsigned ) ulSectorCount );
...
FF_Disk_t * FF_SDDiskInit( const char * pcName )
{
    ...
    do
    {
        ...
        #warning As proposed by Siddhant Modi <siddhant.modi@gmail.com>
...
BaseType_t FF_SDDiskMount( FF_Disk_t * pxDisk )
{
    ...
    {
        char errorStr[ 80 ];
        FF_GetErrDescription( xFFError, errorStr, sizeof( errorStr ) );
        FF_PRINTF( "FF_SDDiskMount: %08lX errno %d (%s)\n", xFFError, prvFFErrorToErrno( xFFError ), errorStr );
        xReturn = 0;
    }
    else
    {
...
eventLogAdd( "ISR %04X %04X", ( unsigned ) ulSDInterruptStatus[ iIndex ], ( unsigned ) ulStatusReg );
...
#if ( ffconfigSDIO_DRIVER_USES_INTERRUPT != 0 )
    void XSdPs_ClearInterrupt( XSdPs * InstancePtr, uint32_t ulMask )
    {
        #if ( ffconfigSDIO_DRIVER_USES_INTERRUPT != 0 )
...
eventLogAdd( "Wait %s %04X", ( ( ulMask & XSDPS_INTR_TC_MASK ) != 0U ) ? "TC" : "CC", ( unsigned ) ulMask );
        configASSERT( iIndex <= 1 );
        ...
        for( ;; )
        {
            ...
            eventLogAdd("Taken %04X %04X", ( unsigned ) ulStatusReg, ( unsigned ) ulSDInterruptStatus[ iIndex ] );
        }
