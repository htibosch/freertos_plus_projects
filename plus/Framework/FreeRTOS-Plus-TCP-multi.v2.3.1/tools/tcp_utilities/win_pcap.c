...
#if ( HEAP_IN_SDRAM == 0 )
    #include "myMalloc.h"
    #warning using heap in SDRAM
    #define malloc( amount, permanent )    myMalloc( amount, permanent )
    #define free( ptr )                    myFree( ptr )
#else
    #warning using freertos heap
    #define malloc( amount, permanent )    pvPortMalloc( amount )
...
/* Get the time in micro seconds. */
    void pcap_checkTime( SPcap * apThis );
...
/*-----------------------------------------------------------*/
        ...
        uint64_t getSystemTime()
        {
            return ullGetHighResolutionTime() + ( ( uint64_t ) 1621758764U * 1000U );
        }
        #define MAX_PATH    129
 ...
 */
                   ...
                   size_t aSize )
    {
        ...
        uint8_t * ptr = ( uint8_t * ) malloc( aSize, pdFALSE );

        if( ptr == NULL )
        {
            lUDPLoggingPrintf( "pcap_init: malloc %u failed\n", aSize );
        }
        else
        {
            ...
            lUDPLoggingPrintf( "pcap_init: Malloc %u ok\n", aSize );
...
/*-----------------------------------------------------------*/
                    ...
                    int aLength )
    {
        ...
        if( ( aLength < 40 ) || ( aLength > ( int ) PCAP_MAX_PACKRET_LENGTH ) )
        {
...
/*-----------------------------------------------------------*/
                         ...
                         const char * pcFname )
    {
        ...
        if( apThis->data == NULL )
        {
            lUDPLoggingPrintf( "pcap_toFile: No buffer was allocated.\n" );
            iResult = -1;
        }
        else if( apThis->xCurrentLength == 0U )
        {
            lUDPLoggingPrintf( "pcap_toFile: nothing to write\n" );
            iResult = 0;
        }
        else
        {
            FF_FILE * writeHandle;
            {
                ...
                if( ptr != NULL )
                {
                    *ptr = '\0';
                    ff_mkdir(
                        fname
                #if( ffconfigMKDIR_RECURSIVE == 1 )
					    , pdTRUE
                #endif
                    );
                }
            }
            writeHandle = ff_fopen( pcFname, "wb" );

            if( writeHandle == NULL )
            {
                lUDPLoggingPrintf( "pcap_toFile: create %s failed: %s\n", pcFname, strerror( errno ) );
                iResult = -errno;
            }
            else
            {
                iResult = ( int32_t ) ff_fwrite( apThis->data, 1, apThis->xCurrentLength, writeHandle );

                if( iResult < 0 )
                {
                    lUDPLoggingPrintf( "pcap_toFile: Write error: %s\n", strerror( errno ) );
                }
                else
                {
                    lUDPLoggingPrintf( "pcap_toFile: %d bytes written to %s\n", ( int ) iResult, pcFname );
                }

                ff_fclose( writeHandle );
            }
        }
...
/*-----------------------------------------------------------*/
    ...
    void pcap_checkTime( SPcap * apThis ) /* Get the time in micro seconds. */
    {
 ...
 */
                       ...
                       const char * pcFname )
    {
        ...
        if( apThis->data == NULL )
        {
            apThis->data = ( SCapData * ) malloc( sizeof( SCapData ), pdFALSE );
        }

        if( apThis->data == NULL )
        {
            lUDPLoggingPrintf( "pcapfile_open: malloc fail\n" );
        }
        else if( apThis->data != NULL )
        {
            memset( apThis->data, 0, sizeof *apThis->data );
            apThis->data->fp = ff_fopen( pcFname, "rb" );

            if( apThis->data->fp == NULL )
            {
                lUDPLoggingPrintf( "pcapfile_open: %s: %s\n", pcFname, strerror( errno ) );
                iResult = -errno;
            }
            else
            {
                struct pcap_hdr_s header;
                int rc = ff_fread( ( char * ) &header, 1, sizeof header, apThis->data->fp );

                if( rc != sizeof header )
                {
                    lUDPLoggingPrintf( "pcapfile_open: Read %d bytes, expected %d\n", rc, sizeof header );
                }
                else if( ( header.magic_number != MAGIC_BE ) && ( header.magic_number != MAGIC_LE ) )
                {
                    lUDPLoggingPrintf( "pcapfile_open: bad magic %X\n", ( unsigned ) header.magic_number );
                }
                else
                {
                    apThis->data->le_convert = ( header.magic_number == PCAP_MAGIC_ID ) ? 0 : 1;
                    lUDPLoggingPrintf( "pcapfile_open: magic %X expect %X convert = %d\n",
                                       ( unsigned ) header.magic_number,
                                       ( unsigned ) PCAP_MAGIC_ID,
                                       ( unsigned ) apThis->data->le_convert );
                    iResult = 1;
                }
                       ...
                       size_t aMaxLen )
    {
        ...
        int rc = ( int ) ff_fread( ( char * ) &( lineHeader ), 1, sizeof lineHeader, apThis->data->fp );
        ...
        {
            lUDPLoggingPrintf( "pcapfile_read: buffer too small: %u < %u\n",
                               ( unsigned ) aMaxLen,
                               ( unsigned ) lineHeader.incl_len );
            pcapfile_close( apThis );
            rc = -1;
        }
        else
        {
            rc = ( int ) ff_fread( ( char * ) apBuf, 1, lineHeader.incl_len, apThis->data->fp );
