 ...
 */
static size_t prvGetMessageFromStreamBuffer( char * pcBuffer,
 ...
 */
static size_t prvBufferFormattedString( const char * pcFormatString,
                                        va_list xArgs );
...
void __attribute__( ( weak ) ) vUDPLoggingHook( const char * pcMessage,
                                                BaseType_t xLength )
{
    ( void ) pcMessage;
    ( void ) xLength;
...
static void prvLoggingTask( void * pvParameters )
{
    ...
    iptraceUDP_LOGGING_TASK_STARTING();
    ...
    #if ( ipconfigMULTI_INTERFACE == 0 )
        {
            ...
            #endif /* if defined( configUDP_LOGGING_ADDR0 ) */
        }
    ...
    for( ; ; )
    {
        ...
        #endif
        {
            ...
            for( lLines = 0; lLines < configUDP_LOGGING_MAX_MESSAGES_IN_BUFFER; lLines++ )
            {
                ...
                #endif /* if ( configUDP_LOGGING_NEEDS_CR_LF != 0 ) */
