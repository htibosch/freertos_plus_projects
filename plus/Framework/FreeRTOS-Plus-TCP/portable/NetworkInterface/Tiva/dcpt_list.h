...
/*  We're using align == 16, ObjSize == 16 */
	...
	size_t _EnetDescriptorsCount( EMAC_DCPT_LIST * pList,
								  BaseType_t isHwCtrl );
...
/* */
	...
	void EMAC_ListAddHead( EMAC_DCPT_LIST * pL,
						   EMAC_DCPT_NODE * pN );
	void EMAC_ListAddTail( EMAC_DCPT_LIST * pL,
						   EMAC_DCPT_NODE * pN,
						   const uint32_t validControl );
	void EMAC_ListRemove( EMAC_DCPT_LIST * pL,
						  EMAC_DCPT_NODE * pN,
						  EMAC_DCPT_NODE * pPrev );
	...
	extern __inline__ void __attribute__( ( always_inline ) ) EMAC_ListAppendTail( EMAC_DCPT_LIST * pL,
																				   EMAC_DCPT_LIST * pAList,
																				   const uint32_t validControl )
	{
		EMAC_DCPT_NODE * pN;
