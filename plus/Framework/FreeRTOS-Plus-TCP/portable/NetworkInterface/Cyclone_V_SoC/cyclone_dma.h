...
#define DMA_INTR_NORMAL						\
	( DMA_INTR_ENA_NIE | DMA_INTR_ENA_RIE |	\
	  DMA_INTR_ENA_TIE )
...
#define DMA_INTR_ABNORMAL					\
	( DMA_INTR_ENA_AIE | DMA_INTR_ENA_FBE |	\
	  DMA_INTR_ENA_UNE )
...
#define DMA_AXI_MAX_OSR_LIMIT							\
	( ( DMA_AXI_OSR_MAX << DMA_AXI_WR_OSR_LMT_SHIFT ) |	\
...
#define DMA_BURST_LEN_DEFAULT			  \
	( DMA_AXI_BLEN256 | DMA_AXI_BLEN128 | \
...
BaseType_t gmac_tx_descriptor_init( int iMacID,
									gmac_tx_descriptor_t * pxDMATable,
									uint8_t * ucDataBuffer,
									uint32_t ulBufferCount );
BaseType_t gmac_rx_descriptor_init( int iMacID,
									gmac_rx_descriptor_t * pxDMATable,
									uint8_t * ucDataBuffer,
									uint32_t ulBufferCount );

void gmac_dma_start_tx( int iMacID,
						uint32_t chan );
void gmac_dma_stop_tx( int iMacID,
					   uint32_t chan );
void gmac_dma_start_rx( int iMacID,
						uint32_t chan );
void gmac_dma_stop_rx( int iMacID,
					   uint32_t chan );

/* Pass the address of the RX descriptor table to DMA. */
void gmac_set_rx_table( int iMacID,
						gmac_rx_descriptor_t * pxDMATable );

/* Pass the address of the TX descriptor table to DMA. */
void gmac_set_tx_table( int iMacID,
						gmac_tx_descriptor_t * pxDMATable );
...
extern void gmac_dma_axi( int iMacID,
						  struct stmmac_axi * axi );
extern void gmac_dma_init( int iMacID,
						   struct stmmac_dma_cfg * dma_cfg );

extern void gmac_dma_operation_mode( int iMacID,
									 int txmode,
									 int rxmode );

extern void gmac_set_dma_interrupt_enable( int iMacID,
										   uint32_t ulMask );
extern uint32_t gmac_get_dma_interrupt_status( int iMacID,
											   int iClear );
extern uint32_t gmac_clear_dma_interrupt_status( int iMacID,
												 uint32_t ulMask );
