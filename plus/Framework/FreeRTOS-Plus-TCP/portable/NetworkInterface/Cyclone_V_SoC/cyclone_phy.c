...
#define PHY_READ_TO_MS						  ( 100U )                /* Max wait time (us) when reading from PHY	*/
#define PHY_WRITE_TO_MS						  ( 100U )                /* Max wait time (us) when reading from PHY	*/
#define PHY_LINKUP_TO_MS					  ( 5000U )               /* Max wait time (us) for link to come up	*/
#define PHY_AUTON_TO_MS						  ( 15000U )              /* Max wait time (us) for auto-ng to succeed*/
...
static int gmac_mdio_read_ext( int PHYid,
							   int iMacID,
							   int PHYaddr,
							   int PHYreg );
static int gmac_mdio_write_ext( int PHYid,
								int iMacID,
								int PHYaddr,
								int PHYreg,
								int PHYval );
...
int cyclone_phy_init( int iMacID,
					  int Rate )
{
	int ii;
	uint32_t MACset;
...
/*			| GMAC_CONTROL_WD;		/ * Watchdog Disable * / */
	...
	if( Rate >= 0 )       /* Request to set the Ethernet speed			*/
	{
		...
		for( Tout = pdMS_TO_TICKS( PHY_LINKUP_TO_MS ); Tout > 0; Tout-- )
		{
